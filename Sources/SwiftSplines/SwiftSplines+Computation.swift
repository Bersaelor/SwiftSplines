...
extension Spline {
    ...
    static func computeDerivatives(
        ...
        withSmoothEndsTridiagonalMatrix(size: Int32(points.count)) { matrix in
            for dimension in 0 ..< P.scalarCount {
                xValues.withUnsafeMutableBufferPointer { xValuesPtr in
                    ...
                    rightHandSize(points.map { $0[dimension] }) { rhs in
                        let status = SparseSolve(SparseConjugateGradient(), matrix, rhs, x, SparsePreconditionerDiagonal)
    ...
    static func rightHandSize(_ points: [P.Scalar], calculate: (DenseVector_Double) -> Void) {
